...
public class WooglieEditor : EditorWindow
{
    ...
    void UploadWebplayer()
    {
        ...
        foreach (string str in Directory.GetFiles(TMPFolder() + "/Wooglie/Release"))
        {
            FileInfo fi = new FileInfo(str);

            if (fi.Extension == ".gz" || fi.Extension == ".datagz" || fi.Extension == ".memgz" || fi.Extension == ".js"  || fi.Extension == ".jsgz")
            {
                string filename = ("file://" + fi.FullName).Replace("\\", "/");
                localFiles.Add(fi.Name, new WWW(filename));
            }
            else if(fi.Extension != ".htaccess")
            {
    ...
    static string GetBuildSizeReport()
    {
        ...
        if (editorLogContents != "")
        {
            ...
            if (lastIndex <= 0) lastIndex = editorLogContents.LastIndexOf("Textures      ");
